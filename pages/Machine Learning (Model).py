...
interested = pd.DataFrame({
    ...
    'RAM': df_sample['RAM'].apply(extract_max_number).fillna(0).astype(float).round().astype(int),
...
interested['Price(Baht)'] = interested['Price(Baht)'].astype(str).str.split(" ",expand=True)[1].str.replace(',', '').astype(float) * 33.64
interested['Price(Baht)'].fillna(interested['Price(Baht)'].median(), inplace=True)
category_mapping = {'Budget': 0, 'Mid-range': 1, 'Premium': 2}
interested['Category'] = interested.apply(categorize_phone, axis=1).map(category_mapping).fillna(0).astype(int)
...
if option == "K-Nearest Neighbors":
    ...
    plot = plt.subplot()
    scatter = plt.scatter(X[ram_col], X[price_col], c=y_pred, cmap=plt.cm.viridis)
    plot.set_xlabel('RAM (GB)')
    plot.set_ylabel('Price (Baht)')
    plot.set_title('Mobile Phone Classification')
    cbar = plt.colorbar(scatter, ticks=[0, 1, 2])
    cbar.set_label("Category (Budget(0) Mid-range(1) Premium(2))")
    plt.show()
    ...
    budget_phones = ({ "Name" : interested[interested["Category"] == 0]["Model Name"]})
    midrange_phones =  ({ "Name" : interested[interested["Category"] == 1]["Model Name"] })
    premium_phones =  ({ "Name" : interested[interested["Category"] == 2]["Model Name"] })
